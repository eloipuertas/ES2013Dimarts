...
(function (f) {
    ...
    d.prototype.createHolders = function () {
        this.$mivHol = f("<div />").addClass("zoomable_holder").css({
            position: "relative",
            overflow: "hidden",
            width: this.sett.viewportWidth,
            height: this.sett.viewportHeight
        });
        this.$preloadHol = f("<div />").addClass("zoomable_preload_holder");
        this.$contentHol = f("<div />").addClass("zoomable_content_holder").css({
            position: "absolute"
        });
        this.$blackScreen = f("<div />").addClass("zoomable_blackScreen").css({
            position: "absolute",
            "z-index": "9999",
            width: "100%",
            height: "100%",
        ...
        if (this.sett.testMode) {
            this.$infoBox = f("<div />").addClass("zoomable_infoBox_holder");
            this.$mivHol.append(this.$infoBox)
        }
    };
    d.prototype.navBttCalcSize = function () {
        var n = 27,
            l = 0,
            o = 4,
            m = this.sett.intNavBttSizeRation;
        if (m > 1) {
            o = Math.ceil(o * m);
            l = o - 4;
            n += 2 * l
    ...
    d.prototype.iniNav = function () {
        var m = f("<ul />").addClass("ui-widget ui-helper-clearfix"),
            ...
            n = this.$navHol,
            q, r, o = this,
            v = this.navBttCalcSize().width,
            u = this.navBttCalcSize().paddingVertical,
            p = this.navBttCalcSize().paddingHoriziontal,
            l = 0,
            s = [
        ...
        f.each(s, function (x) {
            ...
            if (o.sett[B]) {
                l += v;
                A = f("<li />").addClass("ui-state-default ui-corner-all " + y), w = f("<span />").addClass("ui-icon " + s[x][2]);
                A.append(w);
                m.append(A);
                A.css("padding", u + "px " + p + "px");
                A.bind("mouseenter.lhpMIV touchstart.lhpMIV", function () {
                    if (!f(this).hasClass("zoomable_nav_btt_disab")) {
                        f(this).addClass("ui-state-hover")
                    }
                });
                A.bind("mouseleave.lhpMIV touchend.lhpMIV", function () {
                    f(this).removeClass("ui-state-hover")
                });
                A.bind(((o.isTouchDev) ? "touchstart.lhpMIV" : "mousedown.lhpMIV"), function (C) {
                    return function (D) {
                        if (!f(this).hasClass("zoomable_nav_btt_disab")) {
                            t.lhpMegaImgViewer(C)
                        }
                        D.preventDefault()
                    }
                }(y));
                if (z) {
                    A.bind(((o.isTouchDev) ? "touchend.lhpMIV" : "mouseup.lhpMIV"), function (C) {
                        return function (D) {
                            if (!f(this).hasClass("zoomable_nav_btt_disab")) {
                                t.lhpMegaImgViewer(C)
                            }
        ...
        if (this.$navHol.hasClass("zoomable_nav_pos_L") || this.$navHol.hasClass("zoomable_nav_pos_R")) {
            this.$navHol.css("width", v);
            this.$navHol.css("margin-top", -l / 2)
        }
        if (this.$navHol.hasClass("zoomable_nav_pos_T") || this.$navHol.hasClass("zoomable_nav_pos_B")) {
            this.$navHol.css("margin-left", -l / 2)
        }
        t.bind("mivChange.lhpMIV", function (y) {
            var x = "zoomable_nav_btt_disab",
                w = "ui-state-hover";
            if (y.allowDown) {
                n.find(".moveDown").removeClass(x)
            } else {
                n.find(".moveDown").removeClass(w).addClass(x)
            } if (y.allowUp) {
                n.find(".moveUp").removeClass(x)
            } else {
                n.find(".moveUp").removeClass(w).addClass(x)
            } if (y.allowLeft) {
                n.find(".moveLeft").removeClass(x)
            } else {
                n.find(".moveLeft").removeClass(w).addClass(x)
            } if (y.allowRight) {
                n.find(".moveRight").removeClass(x)
            } else {
                n.find(".moveRight").removeClass(w).addClass(x)
            } if (y.allowZoom) {
                n.find(".zoom").removeClass(x);
                n.find(".fullSize").removeClass(x)
            } else {
                n.find(".zoom").removeClass(w).addClass(x);
                n.find(".fullSize").removeClass(w).addClass(x)
            } if (y.allowUnzoom) {
                n.find(".unzoom").removeClass(x);
                n.find(".fitToViewport").removeClass(x)
            } else {
                n.find(".unzoom").removeClass(w).addClass(x);
                n.find(".fitToViewport").removeClass(w).addClass(x)
            }
        });
        if (this.sett.intNavAutoHide) {
            n.css("display", "none");
            t.bind("mouseenter.lhpMIV touchstart.lhpMIV", function () {
                clearInterval(o.navAutohideInterval);
                n.fadeIn("fast")
            });
            t.bind("mouseleave.lhpMIV touchend.lhpMIV", function () {
                clearInterval(o.navAutohideInterval);
                o.navAutohideInterval = setInterval(function (w) {
                    return function () {
                        w.stop().clearQueue().fadeOut("fast")
                    }
                }(n), 1000)
            })
        }
    ...
    d.prototype.imgContentStart = function (l) {
        this.$content = l;
        l.addClass("zoomable_content").css({
            "float": "left"
        });
        this.contentFullSize = {
            w: l.width(),
    ...
    d.prototype.start = function () {
        ...
        if (this.sett.intNavEnable) {
            this.$navHol = f('<div class="zoomable_nav"/>').addClass("zoomable_nav_pos_" + this.sett.intNavPos);
            this.iniNav();
    ...
    d.prototype.mouseenterHandler = function (l) {
        if (!l.data._this.sett.testMode) {
            l.data._this.$contentHol.removeClass("map_cursor_drag").addClass("map_cursor_hand")
        }
    };
    d.prototype.mousedownHandler = function (l) {
        ...
        if (m.isTouchDev) {
            ...
            m.$contentHol.removeClass("map_cursor_hand").addClass("map_cursor_drag");
    ...
    d.prototype.mouseupHandler = function (l) {
        ...
        m.$contentHol.unbind("mouseup.lhpMIV", m.positioning);
        if (!m.sett.testMode) {
            m.$contentHol.removeClass("map_cursor_drag").addClass("map_cursor_hand")
        } else {
    ...
    d.prototype.getSafeScale = function (s) {
        ...
        if (this.sett.fitToViewportShortSide) {
            ...
            //console.log(u);
            $( "#map-scale" ).slider( "option", "value", u );
            $('#active-map').attr('scale', u);
    ...
    d.prototype.getCurrentState = function () {
        ...
        if (this.$content) {
            ...
            m.xPosInCenter = (-r.left + q.x) / p;
            m.yPosInCenter = (-r.top + q.y) / p;
            m.scale = p;
    ...
    b.prototype.ini = function (l) {
        this.$mapHol = f('<div class="zoomable_map"/>');
        this.$mapWrappHol = f('<div class="zoomable_map_wrapp_hol"/>');
    ...
    b.prototype.start = function (m) {
        ...
        this.$mapHol.addClass("zoomable_map_pos_" + this.sett.mapPos).css({
            width: l,
            height: n
        });
        this.$mapWrappHol.addClass("zoomable_map_wrapp_hol_" + this.sett.mapPos).css({
            width: l,
            height: n
        });
        ...
        this.$vr = f('<div class="zoomable_map_vr"/>').css({
            position: "absolute",
            "z-index": 2
    ...
    b.prototype.mouseenterHandler = function (l) {
        l.data._this.$vr.removeClass("map_cursor_drag").addClass("map_cursor_hand")
    };
    b.prototype.mousedownHandler = function (l) {
        ...
        if (m.isTouchDev) {
            ...
            m.$vr.removeClass("map_cursor_hand").addClass("map_cursor_drag");
        ...
        m.lastMousePageCoor = m.mousePageCoor(l);
        m.$vr.addClass("zoomable_map_vr_over");
        l.preventDefault()
    };
    ...
    b.prototype.mouseupHandler = function (l) {
        ...
        if (!m.isTouchDev) {
            m.$vr.removeClass("map_cursor_drag").addClass("map_cursor_hand");
            m.$vr.unbind("mouseenter.lhpMIV", m.mouseenterHandler).bind("mouseenter.lhpMIV", {
                _this: m
            }, m.mouseenterHandler)
        }
        m.$vr.removeClass("zoomable_map_vr_over")
    };
    ...
    var a = function (q, p, l, n, o, m) {
        ...
        this.$contentHol = p;
        this.containerId = l;
        this.mClass = "zoomable_poi";
        this.mInnClass = "zoomable_marker";
        this.pClass = "zoomable_popup";
        this.isTouchDev = n;
        this.markers = [];
        this.popups = [];
    ...
    a.prototype.showPopup = function (l) {
    	...
    	//console.log($(l.$p.context).find('div.content').html());
    	//$('#map-poi-info').html($(l.$p.context).find('div.content').html());
    	
    	$('div#map-poi-info').find('div.default').hide();
    	$('div#map-poi-info').find('div.current').html($(l.$p.context).find('div.content').html()).show();
    ...
    j.prototype.addClose = function () {
        this.$closeHolder = f('<div class="zoomable_popup_close"></div>');
